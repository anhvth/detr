...
class AngularMarginLoss(nn.Module):
    def __init__(self, embed_channels, num_classes,
        ...
        if self.ignore_class0:
            self.num_classes -= 1
        self.weight = nn.Parameter(torch.FloatTensor(self.num_classes, embed_channels))
        ...
        self.ce = nn.CrossEntropyLoss(weight=self.class_weight, reduction='none')
    ...
    def forward(self, embed_feat, labels, weight=None,
        ...
        if self.loss_type == 'arcface':
            ...
            with torch.no_grad():
                ...
                self.t = (1 - self.momentum) * cos_p.mean() + self.momentum * self.t
        ...
        with torch.no_grad():
            ...
            cos_neg = torch.max(torch.where(one_hot.bool(), -1 * torch.ones_like(cosine), cosine), dim=1)[0]
            ...
            # mask_pos = sm_labels.expand(n_pos,n_pos).eq(sm_labels.expand(n_pos,n_pos).t())
            # mask_neg = ~mask_pos
            ...
            # top_idx = top_idx[:, 1:]
            # flat_idx = top_idx.squeeze() + n_pos * torch.arange(n_pos, out=torch.LongTensor()).to(mask_pos)
    ...
    def accuracy(self, embed_feat, labels):
        with torch.no_grad():
            cosine = F.linear(F.normalize(embed_feat), F.normalize(self.weight))
            _, y_pred = torch.max(cosine, dim=1)
            sm_labels = labels.clone()
            if self.ignore_class0:
                # In Obj Detection, class0 is background.
